package com.chakray.esb.mediator;
...
public class EncryptorMediator extends AbstractMediator {
	...
	private boolean process(MessageContext ctx, String payload, String key,
			String bytes) {
		...
		try {
			...
			if (bytes.equals("NULL")) {
				...
				if (key != null && !key.equals("NULL") && bytes != null) {
					...
					return completeProcess(ctx,
							"Encryption key or IVByte String is invalid.");
				}
			}
			return completeProcess(ctx, respondPayload, encryptionKey,
					ivbyteString);
	...
	private boolean completeProcess(MessageContext context, String payload,
			String key, String bytes) {
	...
	private void setResultPayload(MessageContext context, String respondPayload) {
		...
		System.out.println("resultPayload: " + respondPayload);
